...
const ChapterIdPage = async (
    ...
    const requiredFields = [
        chapter.title,
        chapter.videoUrl || chapter.videoSources.length > 0,
    ];

    // Assemble the playable streams and wrap each in a signed proxy URL so the
    // CDN host never appears in the teacher preview's network tab either.
    const playerSources = buildPlayerSources({
        muxPlaybackId: chapter.muxData?.playbackId,
        streams: chapter.videoSources.map((s) => ({
            id: s.id,
            src: s.src,
            title: s.title,
        })),
